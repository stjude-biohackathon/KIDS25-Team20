...
task fq2bam {
  ...
  command <<< 
    ...
    pbrun fq2bam \
      ...
      ~{best_practice_args} \
      --ref "~{ref}" \
      ~{"--knownSites " + known_sites_vcf + " --out-recal-file " + outbase + ".pb.BQSR-REPORT.txt"} \
      --out-bam "~{outbase}".pb.bam
  >>>

  output {
    File bam = "~{outbase}.pb.bam"
    File bai = "~{outbase}.pb.bam.bai"
    File? bqsr = "~{outbase}.pb.BQSR-REPORT.txt" 
  }
